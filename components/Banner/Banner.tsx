...
export const Banner: React.FC<BannerProps> = ({ banner, banner_style }) => {
    ...
    return (
        <>
                            ...
                            <GridBox.GridItem columnMerge={1} className='content-box relative z-[3]'>
                                <div className='title-box relative mb-0 pl-0 pr-[45px] pt-[50px] pb-10'>
                                    ...
                                    <div className='relative xl:top-[50px] lg:top-[50px] md:top-[50px] top-[30px]'>
                                        <button onClick={handlePopup} className='bg-[#FF5500] shadow-[0_10px_30px_rgba(0,0,0,0.1)] text-white text-sm font-bold leading-[30px] overflow-hidden relative uppercase transition-all duration-500 ease-[ease] z-[1] px-[3.7rem] py-[1rem] rounded-none'>{banner.button_label}</button>
                                    </div>
                                </div>
                            </GridBox.GridItem>
            ...
            {open ? (
                <div className={`login ${open ? "fixed inset-0 overflow-y-auto z-[999] bg-[#00000096]" : ""}`}>
