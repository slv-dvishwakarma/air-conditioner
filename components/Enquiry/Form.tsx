...
//             <h2 className="text-3xl font-[700] text-[var(--primary-color)]">{form.cnt_form.title}</h2>
//             <button
...
export const Form: React.FC<FormProps> = ({ form, onClose }) => {
  ...
  const onSubmit = async (data: any) => {
    try {
      const response = await axios.post("https://bareillydeals.com/submit_contact.php", data);
      if (response.status === 200) {
        setMessage("success");
        setTimeout(() => {
          onClose?.();
  ...
  return (
    <>
      {message === "success" ? (
        ...
        <>
          <div className="flex justify-between mb-3 items-center">
            <h2 className="text-3xl font-[700] text-[#FE4236]">{form.cnt_form.title}</h2>
            <button
              className="border  -top-3 -right-3 p-1 text-[var(--primary-color)] border-red-200 hover:border-[var(--primary-color)] active:border-red-100 rounded-full text-[23px]"
              onClick={() => onClose?.()}
            >
