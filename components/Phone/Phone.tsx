...
export const Phone: React.FC<InputProps> = ({
  name,
  placeholder,
  label,
  icon,
    ...
    <div>
      <Controller
        ...
        render={({ field: { onChange, value } }) => (
          <div>
            {label && (
              <label className="text-base font-medium leading-[21px] tracking-[0px] text-left">
                {label}
              </label>
            )}
            <span className="input-border flex items-center border-b-[#BEBEBE] border-b border-solid mt-[12px]">
            <SVGIcon name="mobile" className="text-[var(--primary-color)] text-[14px]" />
              <input
                type="tel"
